  ...
  useEffect(() => {
    async function fetchDestinations() {
      try {
        ...
        setDestinations(data.slice(0, 4)); // Show first 4 destinations
      } catch (error) {
          ...
          {destinations.map((dest, index) => (
            <div
              ...
              className={`relative overflow-hidden rounded-xl group cursor-pointer shadow-sm hover:shadow-lg transition-all duration-500 hover:-translate-y-2
                ${index === 0 ? "lg:col-span-2 lg:row-span-1 h-[300px]" : ""}
                ${index === 1 ? "lg:row-span-2 h-[620px]" : ""}
                ${index === 2 ? "lg:row-span-1 h-[300px]" : ""}
                ${index === 3 ? "lg:row-span-1 h-[300px]" : ""}
              `}
            >
              {/* Destination Image */}
              <img
              ...
              <div className="absolute bottom-5 left-5 text-left text-white">
                ...
                <h3 className="playfair text-[1.75rem] font-[700] text-white leading-[1.4] capitalize mb-2">
                  {dest.name.split(",")[0]}
                </h3>
              </div>
